...
// 	let bo = new Blob([JSON.stringify(data)],{type:"application/json"});//application/json
// 	// 使用 createObjectURL 生成地址，格式为 blob:null/fd95b806-db11-4f98-b2ce-5eb16b38ba36var
// 	let url = URL.createObjectURL(bo);
// 	let a = document.createElement('a');
// 	a.download = String(Date.now()) + '---' + name;
// 	a.href = url; // 模拟a标签点击进行下载
// 	a.click(); // 下载后告诉浏览器不再需要保持这个文件的引用了
// 	URL.revokeObjectURL(url);
// }
// exportJsonFile('st',stObj);
...
function s2t() {
	document.getElementById('jf').innerHTML = document.getElementById('jf').innerHTML.s2t();
}

function t2s() {
	document.getElementById('jf').innerHTML = document.getElementById('jf').innerHTML.t2s();
}
