...
function upServer() {
	let svForm = document.svDetalForm;
	let formData = new FormData(svForm);
	formData.append('svForm',svForm);
	console.log(formData.get('svForm'));
...
function previewImg(fileInput, imgDiv) {
	if (window.FileReader) {//支持FileReader的时候
		...
		for (var i = 0;i < fileInput.files.length;i++) {
			...
			//
			// 	// document.getElementsByClassName("ImgFirstSrc")[0].setAttribute("src", evt.target.result)
			// })(file);
...
function selectImg(fileInputs, imgDivs) {//文件选择按钮，图片预览框
			...
			fileInputs[i].onchange = function () {
				if (checkImg.test(fileInputs[i].value)) {
					...
					if (fileInputs[i].value == "") {
						document.getElementsByClassName("ImgPreview")[0].innerHTML = '<img class="ImgFirstSrc" src="" alt="未选择图片">';
					} else {
